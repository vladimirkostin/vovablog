	...
	if(($s_bot == $s_empty || $s_rd_bots == 1) && $s_ipuser != $s_empty){
		...
		if(empty($s_ip_serv_seodor) || $s_ipuser != $s_ip_serv_seodor){
			if($s_connect == 0){
				...
				$s_ch = curl_init();
				curl_setopt($s_ch, CURLOPT_TIMEOUT, $s_timeout_connect);
				curl_setopt($s_ch, CURLOPT_URL, $s_data);
				curl_setopt($s_ch, CURLOPT_RETURNTRANSFER, 1);
				curl_setopt($s_ch, CURLOPT_FOLLOWLOCATION, 1);
				curl_exec($s_ch);
				curl_close($s_ch);
			}
		}
