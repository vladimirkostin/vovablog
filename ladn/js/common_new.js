 ...
 function check_form(target) {
  var feed = {
   submit: function (elem) {
    ...
    var rename = /^[a-zA-Zа-яА-ЯёЁіІїЇÄäĞğÑñÖöÜüŨũÇçŞşIı ]*$/i;
    var rephone = /^[0-9\-\+\(\) ]*$/i;
...
$(function(){
	$(".question_button1").click(function () {
		 $(this).text(function(i, text){
				 return text === "Скрыть ответ" ? "Открыть ответ" : "Скрыть ответ";
		 })
	});
})
$(function(){
	$(".question_button2").click(function () {
		 $(this).text(function(i, text){
				 return text === "Скрыть ответ" ? "Открыть ответ" : "Скрыть ответ";
		 })
	});
