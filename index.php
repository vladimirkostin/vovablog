...
if(!isset($_GET['api'])){
	...
	if(preg_match("~^$path\/(.+?)\?.+?=(.+?)?$~", $in_url, $matches)){
		...
		if(empty($parameters)){
			if(!empty($par[0])){$parameter_1 = urldecode(trim($par[0]));}
			if(!empty($par[1])){$parameter_2 = urldecode(trim($par[1]));}
			if(!empty($par[2])){$parameter_3 = urldecode(trim($par[2]));}
			if(!empty($par[3])){$parameter_4 = urldecode(trim($par[3]));}
			if(!empty($par[4])){$parameter_5 = urldecode(trim($par[4]));}
		}
		if(!empty($parameters)){
			$parameters = @unserialize($parameters);
			if(!empty($parameters['key'])){$key = $parameters['key'];}
			if(!empty($parameters['referer'])){$referer = $parameters['referer'];}
			if(!empty($parameters['par_1'])){$parameter_1 = urldecode(trim($parameters['par_1']));}
			if(!empty($parameters['par_2'])){$parameter_2 = urldecode(trim($parameters['par_2']));}
			if(!empty($parameters['par_3'])){$parameter_3 = urldecode(trim($parameters['par_3']));}
			if(!empty($parameters['par_4'])){$parameter_4 = urldecode(trim($parameters['par_4']));}
			if(!empty($parameters['par_5'])){$parameter_5 = urldecode(trim($parameters['par_5']));}
		}
	}
...
if(isset($id) && !empty($id) && file_exists($ini_folder.'/'.$id.'.ini')){
...
if(!empty($s_name)){
	...
	if($bot == $empty && $s_ch_domain_name == 1 && $ipuser != $empty){
...
exit();
function out(){
	global $redirect, $out, $key, $lang, $country, $city, $region, $device, $operator, $bot, $uniq, $path, $s_chance, $g_save_keys_se, $parameter_1, $parameter_2, $parameter_3, $parameter_4, $parameter_5, $key_se, $ipuser, $api_key, $api_key_host, $debug, $domain, $useragent, $id, $cid, $cid_delimiter, $curl_cache, $s_name, $g_name, $header;
	...
	if(stristr($out, 'RANDNUM') || stristr($out, 'RANDSTR') || stristr($out, 'RANDLINE')){
		if(preg_match("~\[RANDNUM-([0-9]*)-([0-9]*)\]~", $out, $matches)){
			$rand_num = mt_rand($matches[1], $matches[2]);
			$out = preg_replace('~\[RANDNUM-[0-9]*-[0-9]*\]~', $rand_num, $out);
		}
		if(preg_match("~\[RANDSTR-\((.+?)\)-([0-9]*)\]~", $out, $matches)){
			$str = $matches[1];
			$count = $matches[2];
			$strl = strlen($str) - 1;
			$rand_str = '';
			while(!empty($count)){
				$sym = $str{mt_rand(0, $strl)};
				$rand_str = $rand_str.$sym;
				$count--;
			}
			$out = preg_replace('~\[RANDSTR-\((.+?)\)-([0-9]*)\]~', $rand_str, $out);
		}
		if(preg_match("~\[RANDLINE-\((.+?)\)\]~", $out, $matches)){
			$rf = $matches[1];
			$rf = file("database/$rf");
			$rand_str = trim($rf[array_rand($rf)]);
			$out = preg_replace('~\[RANDLINE-\((.+?)\)\]~', $rand_str, $out);
		}
	}
	if($redirect == 'api'){
		if($debug != 1){
...
function search_in_database($networks, $ip){
	...
	for($i = 0; $i<count($networks); $i++){
		...
		if(trim($networks[$i]) != false && trim($networks[$i]{0}) != '#'){
			$networks[$i] = trim($networks[$i]);
			if($ip == $networks[$i]){
				return true;
			}
		}
		if(trim($networks[$i]) != false && trim($networks[$i]{0}) != '#' && strstr($networks[$i], '/')){
...
function logs(){
	global $id, $g_name, $s_name, $log_folder, $out, $key, $redirect, $device, $operator, $country, $city, $region, $lang, $uniq, $bot, $ipuser, $referer, $useragent, $domain, $se, $g_log, $log_save, $db, $empty, $log_bots, $log_out, $log_ref, $log_key, $log_ua, $counter, $timeout, $postback, $cid, $cid_length, $index;
	...
	if($g_log == 1){
		...
		$table = strtotime(date('d-m-Y'));
		if(!$db->querySingle("SELECT * FROM sqlite_master WHERE type = 'table' AND name = '$table';")){
			$db->querySingle("PRAGMA encoding = 'UTF-8'; PRAGMA journal_mode = WAL; CREATE TABLE '$table' (id INTEGER PRIMARY KEY, date TEXT, time TEXT, ngroup TEXT, nstream TEXT, out TEXT, keyword TEXT, redirect TEXT, device TEXT, operator TEXT, country TEXT, city TEXT, region TEXT, lang TEXT, uniq TEXT, bot TEXT, ipuser TEXT, referer TEXT, useragent TEXT, domain TEXT, se TEXT, strtotime INTEGER, counter TEXT, cid TEXT, postback TEXT);");
 			if($index == 1 || $index == 2){
...
function trash(){
	global $trash_url, $trash;
	if(empty($trash_url) || $trash == 0){
		exit();
	}
	if($trash == 1){
...
function curl($out, $key, $file_name){
	global $curl_ua, $curl, $curl_cache, $s_name, $g_name;
	$ch = curl_init($out);
	@curl_setopt($ch, CURLOPT_HEADER, 0);
