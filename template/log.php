	...
	if($db->querySingle("SELECT * FROM sqlite_master WHERE type = 'table' AND name='$table'")){
		if($q == 'log'){
			if(empty($s_name) && $_GET['pb'] != 'on'){
				...
				$res = $db->query("SELECT * FROM '$table' $qd $order LIMIT $log_limit;");
			}
			if(!empty($s_name) && $_GET['pb'] != 'on'){
				if(!empty($d)){
					$qd = "AND domain = '$d'";
				}
				$res = $db->query("SELECT * FROM '$table' WHERE nstream = '$s_name' $qd $order LIMIT $log_limit;");
			}
			if(empty($s_name) && $_GET['pb'] == 'on'){
				if(!empty($d)){
					$qd = "AND domain = '$d'";
				}
				$res = $db->query("SELECT * FROM '$table' WHERE postback != '$empty' $qd $order LIMIT $log_limit;");
			}
			if(!empty($s_name) && $_GET['pb'] == 'on'){
				if(!empty($d)){
					$qd = "AND domain = '$d'";
				}
				$res = $db->query("SELECT * FROM '$table' WHERE postback != '$empty' $qd AND nstream = '$s_name' $order LIMIT $log_limit;");
			}
			echo "<!DOCTYPE html>
<html>
<head>
...
<div class=\"align_center bold\">Group: $g_name | Stream: $s_name"; if(!empty($d)){echo " | Domain: $d";} echo '</div>';}
			else{echo "
<div class=\"align_center bold\">Group: $g_name"; if(!empty($d)){echo " | Domain: $d";} echo '</div>';}
			echo '<br>
<center><a href="'.$admin_page.'">Main</a> | <a href="javascript:location.reload();">Update</a> | <a href="javascript:history.back();">Back</a> | <a href="'.$admin_page.'?q=export&g='.$g_id.'&s='.$s.'&d='.$d.'&t='.$table.'&pb='.$_GET['pb'].'">Export</a></center><br>';
			if(file_exists($log_folder.'/'.$g_id.'.db')){
				$option = '';
				$a = 0;
				while($a != $log_days){
					$table_temp = strtotime(date("d-m-Y", strtotime('-'.$a.' day')));
...
<input name="s" type="hidden" value="'.$s.'">';
			...
			echo '
<input name="pb" class="checkbox" type="checkbox"'; if($_GET['pb'] == 'on'){echo ' checked="checked"';} echo '><span>Postback</span>
<select type="hidden" name="t" size = "1">';
			echo $option;
			echo '
</select>';
			echo '
<select type="hidden" name="o" size = "1">
...
<input style="max-width:30px;" name="n" type="text" value="'.$log_limit.'" maxlength="7">
<input style="width:50px;height:22px;" type="submit" value="View">
</form><br>';
			echo '
...
<td class=\"log\">".htmlentities($array['keyword'], ENT_QUOTES, 'UTF-8')."</td>
...
<td class=\"log\">".htmlentities($array['domain'], ENT_QUOTES, 'UTF-8')."</td>
<td class=\"log\">".htmlentities(urldecode($array['referer']), ENT_QUOTES, 'UTF-8')."</td>
<td class=\"log\">".htmlentities($array['useragent'], ENT_QUOTES, 'UTF-8')."</td>
<td>".$array['se']."</td>
<td>".$pb."</td>
</tr>";
			}
...
<div class="debug">'.(microtime(true) - $start).' s.</div>';
		...
		if($q == 'export'){
			if(empty($s_name) && $_GET['pb'] != 'on'){
				...
				$res = $db->query("SELECT * FROM '$table' $qd;");
			}
			if(!empty($s_name) && $_GET['pb'] != 'on'){
				if(!empty($d)){
					$qd = "AND domain = '$d'";
				}
				$res = $db->query("SELECT * FROM '$table' WHERE nstream = '$s_name' $qd;");
			}
			if(empty($s_name) && $_GET['pb'] == 'on'){
				if(!empty($d)){
					$qd = "AND domain = '$d'";
				}
				$res = $db->query("SELECT * FROM '$table' WHERE postback != '$empty' $qd;");
			}
			if(!empty($s_name) && $_GET['pb'] == 'on'){
				if(!empty($d)){
					$qd = "AND domain = '$d'";
				}
				$res = $db->query("SELECT * FROM '$table' WHERE postback != '$empty' AND nstream = '$s_name' $qd;");
			}
