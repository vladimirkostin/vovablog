...
else{$table = strtotime(date('d-m-Y'));}
...
if(isset($_POST['g_id']) AND $_POST['button'] == "Submit"){
	...
	if(isset($_POST['g_get']) && !empty($_POST['g_get'])){$g_get = $_POST['g_get'];}
	else{$g_get = '';}
	...
	foreach($files as $v){
		if($v != "." && $v != ".." && $v != ".htaccess"){
			$a = unserialize(file_get_contents($ini_folder.'/'.$v));
	...
	if(empty($error)){
		...
		if(!empty($g_get) && $g_id != $g_get && file_exists($ini_folder.'/'.$g_get.'.ini')){
			$g_data = unserialize(file_get_contents($ini_folder.'/'.$g_get.'.ini'));
		}
		$g_data[0]['g_id'] = $g_id;
